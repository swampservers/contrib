...
function load_swep(name, callback)
    _G.SWEP = {Base="weapon_base",Folder="weapons/"..name,Primary={},Secondary={}}
    callback()
...
function load_effect(name, callback)
    if CLIENT then _G.EFFECT = {Folder="effects/"..name} end
    callback()
    if CLIENT then 
...
local function auto_file(fn)
    ...
    for i,v in ipairs(pathparts) do
        if v:StartWith("cl_") then assert(class==nil) class="cl" end
        if v:StartWith("sh_") then assert(class==nil) class="sh" end
        if v:StartWith("sv_") then assert(class==nil) class="sv" end
    end
    ...
    if basefn:StartWith("ent_") or topdir=="entities" then  assert(class==nil) class="ent" end
    if basefn:StartWith("weapon_") or topdir=="weapons" then  assert(class==nil) class="swep" end
    if topdir=="effects" then  assert(class==nil) class="effect" end
    ...
    elseif class=="ent" then
        load_ent(basefn, function() try(sh_file, fn) end)
    elseif class=="swep" then
        load_swep(basefn, function() try(sh_file, fn) end)
    elseif class=="effect" then
        load_effect(basefn, function() try(cl_file, fn) end)
    else
    ...
    for i, d in ipairs(dirs) do
        -- if not d:StartWith("_") then 
            if d:StartWith("ent_") or dir:EndsWith("/entities") then
                load_ent(d, function() try(sv_file, dir .. "/" .. d.."/init.lua") try(cl_file, dir .. "/" .. d.."/cl_init.lua") end)
            elseif d:StartWith("weapon_") or dir:EndsWith("/weapons") then
                load_swep(d, function() try(sv_file, dir .. "/" .. d.."/init.lua") try(cl_file, dir .. "/" .. d.."/cl_init.lua") end)
            elseif dir:EndsWith("/effects") then
                load_effect(basefn, function() try(cl_file, dir .. "/" .. d.."/init.lua") end)
            else
                Load(dir .. "/" .. d)
            end
        -- end
