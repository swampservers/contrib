...
concommand.Add("bp", function( ply, cmd, args )
    ...
    if IS_RETARDED then
        ...
        function MAKE_RETARDED(tab,scope)
            ...
            for k,v in pairs(tab) do
                if isfunction(v) then
                    ...
                    tab[k] = function(...)
                        if coroutine.running() then print("CO") return funt(...) end
                        RP_PUSH(name)
                        local ret={funt(...)}
                        RP_POP()
                        return unpack(ret)
        ...
        TIMER_DETOUR = function(name, funt)
            RP_PUSH(name)
            funt()
            if #RP_STACK>0 then RP_POP() end --it gets reset by the printer
        end
...
function timer.Simple(t,f)
    ...
    RP_ACTUAL_TIMERSIMPLE(t, function()
        if TIMER_DETOUR then TIMER_DETOUR(name, fonc) else fonc() end
    end)
end
