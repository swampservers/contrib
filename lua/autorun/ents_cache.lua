...
hook.Add("EntityRemoved", "Ents_EntityRemoved", function(v)
    ...
    if not v.EntsCacheIndex then print("FUCK",v) return end
...
function _TestEnts()
    ...
    for k, v in pairs(Ents) do
        if table.Count(v) > 0 then
            ...
            local svl = {}
            for sk2,sv2 in pairs(allents) do -- ents.FindByClass(k) is different...
            ...
            for k2, v2 in pairs(v) do
                ...
                -- assert(v2 == sv2)
                assert(IsValid(v2))
                if not svl[v2] then print(v2, v2:EntIndex(), v2.EntsCacheIndex, table.Count(Ents[k]), k, v2:GetClass(), #ents.FindByClass(k)) end
                assert(svl[v2])
            end
...
if SERVER or IsValid(LocalPlayer()) and LocalPlayer():Nick()=="Joker Gaming" 
then _TestEnts() end
end)
