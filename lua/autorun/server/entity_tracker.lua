...
hook.Add("OnEntityCreated", "_entity_counts_add", function(ent)
    ...
    if _CacheEntityCount(class) then -- It seems to already be added to ents.FindByClass
...
hook.Add("EntityRemoved", "_entity_counts_subtract", function(ent)
    local class = ent:GetClass()
    if _CacheEntityCount(class) then -- It seems to already be removed from ents.FindByClass
        _entity_counts[class] = _entity_counts[class] - 1
    end
end)
