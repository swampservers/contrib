    ...
    for k, v in pairs(MINECRAFT_EXPOSED_ORES or {}) do
        if v >= 3 then
            ...
            if dlight then
                ...
                local of = 0.6
                if cvx_in_world(x+1,y,z) and not cvx_get_vox_solid(x+1,y,z) then cx=cx+of end
                if cvx_in_world(x-1,y,z) and not cvx_get_vox_solid(x-1,y,z) then cx=cx-of end
                if cvx_in_world(x,y+1,z) and not cvx_get_vox_solid(x,y+1,z) then cy=cy+of end
                if cvx_in_world(x,y-1,z) and not cvx_get_vox_solid(x,y-1,z) then cy=cy-of end
                if cvx_in_world(x,y,z+1) and not cvx_get_vox_solid(x,y,z+1) then cz=cz+of end
                if cvx_in_world(x,y,z-1) and not cvx_get_vox_solid(x,y,z-1) then cz=cz-of end
