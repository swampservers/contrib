...
function MvisNextFrame(...)
    ...
    local fftdelta = {}
    for i = 1,#fft1 do table.insert(fftdelta,fft1[i]-((l.fft1 or {})[i] or 0)) end
    local fftds, fftdsa = 0,0
    for i = 1,#fft1 do fftds=fftds+fftdelta[i] fftdsa=fftdsa+math.abs(fftdelta[i]) end
    ...
    if UsingMusicVis("flash") then t.drive=t.drive+0.8 end
    if UsingMusicVis("dark") or UsingMusicVis("colorful") then t.drive=0 end
    ...
    t.flash = 0
    if FORCEDRIVE then t.drive=FORCEDRIVE end
    ...
    if UsingMusicVis("flash") then t.flash=t.flash*1.5 end
...
function MvisPlot(name, mean0, t0)
    ...
    if not chart then
        table.insert(MVIS_PLOTS,{name=name,mean0=mean0,lookback=lookback,chart={},t0=t0})
...
function MvisDrawDebug()
    ...
    for j,tab in ipairs(MVIS_PLOTS) do
        ...
        if mean0 then surface.DrawLine(x1, (y1 + y2) / 2, x2, (y1 + y2) / 2) end
