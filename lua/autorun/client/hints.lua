...
function SetSwampHint(to, timelimit)
    if CURRENT_SWAMP_HINT then SWAMP_HINTS[CURRENT_SWAMP_HINT] = nil end --unset hints once they're shown
...
hook.Add("Think","SwampHintThink", function()
    ...
    if not CURRENT_SWAMP_HINT then
        if lp.InTheater and lp:InTheater() then SetSwampHint("theater") end
        -- if not lp:Alive() then SetSwampHint("store") end
        if CurTime()-SWAMP_HINT_JOIN_CURTIME > 40 then SetSwampHint("store") end
    elseif CURRENT_SWAMP_HINT == "intro" then
        local b = input.LookupBinding("showscores")
        if b and input.IsButtonDown(input.GetKeyCode(b)) then SetSwampHint("settings") end
    elseif CURRENT_SWAMP_HINT == "settings" then
        local b = input.LookupBinding("showscores")
        if b and not input.IsButtonDown(input.GetKeyCode(b)) then SetSwampHint() end
    elseif CURRENT_SWAMP_HINT == "theater" then
        local b = input.LookupBinding("menu")
        if b and input.IsButtonDown(input.GetKeyCode(b)) then SetSwampHint() end
    elseif CURRENT_SWAMP_HINT == "store" then
        if input.IsKeyDown(KEY_F3)  then SetSwampHint() end
    end
end)
