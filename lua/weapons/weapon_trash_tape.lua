...
function SWEP:PrimaryAttack()
	...
	if CLIENT then
		if PropTrashLookedAt then
			...
			net.WriteTable({ent=PropTrashLookedAt,act=TRASHACT_TAPE,hitpos=PropTrashLookedAtPos})
			net.SendToServer()
		end
	end
end
...
function SWEP:SecondaryAttack()
	...
	if CLIENT then
		if PropTrashLookedAt then
			net.Start("TrashAction")
			net.WriteTable({ent=PropTrashLookedAt,act=TRASHACT_UNTAPE,hitpos=PropTrashLookedAtPos})
			net.SendToServer()
		end
	end
end
...
function SWEP:Reload()
	if CLIENT then
		...
		if PropTrashLookedAt then
			net.Start("TrashAction")
			net.WriteTable({ent=PropTrashLookedAt,act=TRASHACT_REMOVE,hitpos=PropTrashLookedAtPos})
			net.SendToServer()
		else
			PS_BuyProduct('trash')
		end
