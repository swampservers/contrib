...
local function sinusoidlerp(lrp)
	return 0.5 - (0.5*math.cos(lrp*2.9)) --*math.pi
end
...
function SWEP:TargetedPlayer()
	...
	for k,v in pairs(allply) do
		...
		if self:TraceCapsule(vp, vr, v:GetPos(), ca, 12, v:Crouching() and 38 or 58, 12.5, 100) then --radius was 12
...
function SWEP:PrimaryAttack()
	...
	if shouldHearChink then self:ExtEmitSound("Weapon_Knife.HitWall",{pitch=100, shared=true}) end
	self.Owner:SetAnimation( PLAYER_ATTACK1 )
end

if SERVER then
	--file to duration
	slitterTaunts = { ["throatneck.ogg"]=1.3,["throatneck2.ogg"]=2.1,} --["illgetyou.ogg"]=0.95 }
end
...
function SWEP:SecondaryAttack()
	if SERVER then
		...
		self:ExtEmitSound(s,{pitch=p, speech=slitterTaunts[s], channel=CHAN_VOICE})
...
function SWEP:DrawHUD()
	...
	if killeffect<0.98 then
		...
		if (CurTime()-lastkillslit)>=0.35 then justslitplayer = nil end
	end
