...
function SWEP:PrimaryAttack()
    if SERVER then
        ...
        if thisautism == lastAutism then
            thisautism = math.random(1, 11)
        end
        ...
        if stime > 2 then
            timer.Simple(math.Rand(0, 2), function()
                if IsValid(self.Owner) and not self.Owner.autismWalkSpeed then
                    setPlayerGesture(self.Owner, GESTURE_SLOT_ATTACK_AND_RELOAD, table.Random(autisticTwitches), true)
                end
            end)
        end
        ...
        if stime > 4 then
            timer.Simple(math.Rand(3, 4), function()
                if IsValid(self.Owner) and not self.Owner.autismWalkSpeed then
                    setPlayerGesture(self.Owner, GESTURE_SLOT_ATTACK_AND_RELOAD, table.Random(autisticTwitches), true)
                end
            end)
...
function SWEP:Reload()
    if CurTime() > (self.Owner.autismlastreload or 0) + 2 then
        ...
        if self.Owner:KeyDown(IN_WALK) then
            self:ExtEmitSound("autism/takeaway.wav", {
                speech = -1,
                pitch = pit
            })
        else
            ...
            timer.Simple(0.4, function()
                self:ExtEmitSound("autism/hai.wav", {
                    speech = -1,
                    pitch = pit
                })
            end)
