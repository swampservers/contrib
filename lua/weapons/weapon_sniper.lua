...
function SWEP:PrimaryAttack() 
	...
	if SERVER or IsFirstTimePredicted() then 
		timer.Simple(0.2, function() self:SetNWInt("sc", 0) end)
	end
	local bullet = {} 
	...
	local rndb = self.Primary.Recoil * math.random(-1, 1) 
		...
		timer.Simple( 0.5/BOLTACTIONSPEED, function() if IsValid(self) and IsValid(self.Owner) then self.Owner:EmitSound( BoltBack ) end end )
		timer.Simple( 0.9/BOLTACTIONSPEED, function() if IsValid(self) and IsValid(self.Owner) then self.Owner:EmitSound( BoltForward ) end end)
...
if wep:Clip1() == 5 or owner:GetAmmoCount( self.Primary.Ammo ) == 0 then return false end
	if wep:Clip1() < 5 then
		...
		timer.Simple( 0, function () self:EmitSound("weapons/smg1/smg1_reload.wav") end)
			timer.Simple( 2.2, function ()  if IsValid(self) and IsValid(self.Owner) then  self:EmitSound( BoltBack ) end end )
			timer.Simple( 2.7, function ()  if IsValid(self) and IsValid(self.Owner) then self:EmitSound( BoltForward ) end end )
	end
end
