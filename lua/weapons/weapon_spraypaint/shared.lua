...
local function doSpray(decal,self) 
	...
	if decal=="BulletProof" then interdist = 1.3 end
	...
	PaintPlaceDecal( self:GetOwner(), trace.Entity, { Pos1 = Pos1, Pos2 = Pos2, bone = trace.PhysicsBone, decal = decal } )
...
local out = true
if self.Owner:GetVelocity():Length()>100 and sprayLastVelocity<=100 then sprayFastStart=self.Owner:GetPos() end
if self.Owner:GetVelocity():Length()>100 and self.Owner:GetPos():Distance(sprayFastStart)>200 then out = false end
