...
function SWEP:PrimaryAttack()
	...
	timer.Simple(2, function()
		if IsValid(self) and IsValid(self.Owner) then
			self:ExtEmitSound("weapon_funnybanana/audiencelaugh.ogg", {shared=true, level=65, volume=0.7, channel=CHAN_AUTO})
			self:ExtEmitSound("weapon_funnybanana/slipsoundc.ogg", {shared=true, level=65, volume=0.5, channel=CHAN_AUTO})
			self:ExtEmitSound("weapon_funnybanana/"..cartoonsnd[math.random(#cartoonsnd)], {shared=true, level=65, volume=0.4, channel=CHAN_AUTO})
			self:ExtEmitSound("airhorn/honk1.ogg", {shared=true, level=65, volume=0.5, channel=CHAN_AUTO})
		end
	end)
