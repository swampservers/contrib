...
if(!state)then clicksound = self.UnClickSound end
...
function SWEP:Deploy()
	if(CLIENT)then self.HornBG = nil end
	self:UpdateVMFOV()
	if(SERVER and self:GetOwner().laserpointerlastcolor)then self:SetColorMode(self:GetOwner().laserpointerlastcolor) end
	return true
end
...
if(self.ColorFunctions[nextmode] == nil)then nextmode = 0 end
...
function SWEP:PrimaryAttack( )
	if(self:Clip1() <= 0)then return true end
		...
		local take = 1 
		if(self:GetBeamMode())then take = 4 end
		self:SetClip1(math.max(self:Clip1() - take,0))
		if(self:GetBeamMode())then LaserPointer_SVBeam(ply,self,ply:EyePos(),ply:GetAimVector()) end
...
hook.Add("HUDPaintBackground","laserpointer_blinding",function()
...
function LaserPointer_DrawBeam(ply,wep,origin,dir,color,phase,startoverride)
		if(!CLIENT or !IsValid(ply) or !IsValid(wep) or origin == nil or dir == nil or color == nil)then return end
		phase = phase or 0
		if(phase >= 15)then return end
		local trace = {}
		trace.start = origin
		trace.endpos = origin + (dir * 20000) 
		if(phase == 0)then trace.filter = ply end
		...
		if((tr.HitWorld or tr.Hit) and !tr.HitSky)then
...
function LaserPointer_SVBeam(ply,wep,origin,dir,phase)	-- for damage!
	if(!SERVER or !IsValid(ply) or !IsValid(wep) or origin == nil or dir == nil)then return end
	phase = phase or 0
	if(phase >= 15)then return end
	local trace = {}
	trace.start = origin
	...
	if(tr.HitWorld or tr.Hit)then
		...
		if(reflect)then
			...
			if(IsValid(tr.Entity) and tr.Entity.Health != nil)then
				if(Safe == nil or (isfunction(Safe) and !Safe(tr.Entity)))then
					local d = DamageInfo()
					d:SetDamage( 1 )
					d:SetAttacker( ply )
					d:SetInflictor( wep )
...
local laserhook = function(depth,skybox)
	...
	if(laser_material)then
		for wep,_ in next, lpointer_laser_source do
			...
			if(!wep:GetOnState())then continue end
...
function SWEP:GetLaserColor()
	...
	if(IsValid(ply))then
		...
		if(cfunc)then
		if(IsColor(cfunc))then lasercolor = cfunc end
		if(type(cfunc) == "function")then lasercolor = cfunc(ply) end
...
function SWEP:PostDrawViewModel( vm, weapon, ply )
	...
	cam.Start3D2D( pos, ang, 0.008 )
		...
		if(self:Clip1() > self:GetMaxClip1()*0.1 or math.sin(math.rad(CurTime()*720)) > 0)then
			draw.SimpleText( math.Round((self:Clip1()/self:GetMaxClip1())*100,0).."%", "Trebuchet24", 0,9, self:GetLaserColor(), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			else
			local indcolor = table.Copy(self:GetLaserColor())
			draw.SimpleText( math.Round((self:Clip1()/self:GetMaxClip1())*100,0).."%", "Trebuchet24", 0,9, indcolor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )	
		end
		...
		local indcolor = table.Copy(self:GetLaserColor())
		if(!self:GetBeamMode())then indcolor.a = 64 end		
...
function SWEP:DrawWorldModel(query)
	...
	if IsValid(ply) then
		...
		if(isPony and self.HornBG == nil)then
			...
			if(self.HornBG == -1)then self.HornBG = ply:FindBodygroupByName( "horn" ) end
		end
		if(self.HornBG != nil and self.HornBG != -1 and ply:GetBodygroup(self.HornBG) == 0)then horn = true  end
		...
		if mrt then
			...
			mrt:SetAngles(oang)
			if(query != true)then self:SetBoneMatrix(0, mrt) end
		end
	end
	if(query != true and !horn)then 
