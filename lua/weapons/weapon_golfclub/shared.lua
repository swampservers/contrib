...
function SWEP:Reload()
    if (SERVER) then
        if IsValid(self.ActiveBall) then
            --timing glitch
            if self.ActiveBall.FineHole then
                return
            end
...
function SWEP:PrimaryAttack()
    ...
    if (SERVER) then
        ...
        if (stage == 0) then
            ...
            if (spoint ~= nil and not IsValid(self.ActiveBall)) then
                can = true
                local ball = ents.Create("golfball")
                ball:SetPos(spoint + Vector(0, 0, -2))
                if spoint:Distance(Vector(5587, 1638, -3210)) < 1 then
                    ball.NoBounds = true
                end
        ...
        if
            (stage == 2 and IsValid(self:GetBallToShoot()) and
                not self:GetBallToShoot():GetPhysicsObject():IsMotionEnabled())
         then
...
function SWEP:FinishStroke(hole)
    ...
    if (hole) then
        ...
        GolfNotify(
            "[yellow]" .. self:GetOwner():Nick() .. " completed hole '[gold]" .. hole .. "[yellow]' with " .. endm
        )
        if SERVER then
            ...
            if strk < record then
                ...
                self:GetOwner():PrintMessage(
                    HUD_PRINTTALK,
                    "[red]You beat your all-time record! New record: [orange]" .. tostring(strk) .. strokes
                )
                alltime = true
            end
            strokes = " strokes"
            if strk < periodicrecord then
                ...
                if not alltime then
                    self:GetOwner():PrintMessage(
                        HUD_PRINTTALK,
                        "[red]You beat your weekly record! New record: [orange]" .. tostring(strk) .. strokes
                    )
                end
                    ...
                    for k, v in pairs(t) do
                        if periodicrecord > v[1] and strk <= v[1] then
                            if self:GetOwner().familyshared then
                                self:GetOwner():PrintMessage(
                                    HUD_PRINTTALK,
                                    "[red]Sorry, you're not eligible for this prize ;hahaha;"
                                )
                            else
                                self:GetOwner():PrintMessage(
                                    HUD_PRINTTALK,
                                    "[red]Congratulations! You won [gold]" .. tostring(v[2]) .. " points!"
                                )
                ...
                if periodicrecord == 1 then
                    strokes = " stroke"
                end
                self:GetOwner():PrintMessage(
                    HUD_PRINTTALK,
                    "[red]Your all-time record for this hole is: [orange]" ..
                        tostring(record) ..
                            strokes .. " [red]([orange]" .. tostring(periodicrecord) .. strokes .. " [red]this week)"
                )
            end
            local t = GolfPrizeTargets[hole]
            if t then
                ...
                if nex == nil then
                    ...
                    self:GetOwner():PrintMessage(
                        HUD_PRINTTALK,
                        "[red]Beat this hole in [orange]" ..
                            tostring(nex[1]) .. strokes .. "[red] to win [gold]" .. tostring(nex[2]) .. " points!"
                    )
                end
            end
        end
