...
function SWEP:DrawWorldModel()
	...
	if IsValid(ply) then
		...
		if ply:IsPony() then
			--pony position
			opos = opos + (oang:Forward()*6.5) + (oang:Right()*-4.36) + (oang:Up()*-2.5)
...
function SWEP:GetViewModelPosition(pos, ang)
	...
	if not LocalPlayer().monsterArmTime then LocalPlayer().monsterArmTime=0 end
	local lerp = math.Clamp((os.clock()-LocalPlayer().monsterArmTime)*3,0,1)
	if LocalPlayer().monsterArm then lerp = 1-lerp end
