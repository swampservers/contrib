...
hook.Add("CreateMove", "golfswinger", function(cmd) end) -- if GOLFCAMVECTOR then --     chat.AddText(tostring(cmd:GetMouseX())) --     GOLFSWINGX = math.Clamp(GOLFSWINGX+cmd:GetMouseX()*-0.0005,-1,1) --     GOLFSWINGY = math.Clamp(GOLFSWINGY+cmd:GetMouseY()*0.0005,-1,1) --     cmd:SetViewAngles(GOLFCAMVECTOR:Angle()) --     cmd:SetForwardMove(0) --     cmd:SetSideMove(0) -- end
...
function SWEP:DrawHUD()
    ...
    if GOLFCAMVECTOR and false then
        ...
        surface.DrawLine(xc - GOLFSWINGX * GOLFHUDSCALE, yc + GOLFSWINGY * GOLFHUDSCALE + GOLFCLUBRADIUS * GOLFHUDSCALE, xc - GOLFSWINGX * GOLFHUDSCALE, yc + GOLFSWINGY * GOLFHUDSCALE - GOLFCLUBRADIUS * GOLFHUDSCALE)
        local strike_offset = math.Clamp(-GOLFSWINGY, -GOLFCLUBRADIUS, GOLFCLUBRADIUS)

        if GOLFSWINGFRAMES[1] then
            surface.DrawLine(xc - GOLFSWINGX * GOLFHUDSCALE, yc + GOLFSWINGY * GOLFHUDSCALE + strike_offset * GOLFHUDSCALE, xc - GOLFSWINGFRAMES[1][1] * GOLFHUDSCALE, yc + GOLFSWINGFRAMES[1][2] * GOLFHUDSCALE + strike_offset * GOLFHUDSCALE)
        end
        -- surface.DrawLine(
        ...
        if stage == 2 then
            if self:WeirdGolf() then
                if not GOLFCAMVECTOR then
                    ...
                    if bpos.z > EyePos().z then
                        cir[1], cir[2] = cir[2], cir[1]
                    end
