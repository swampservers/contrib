...
function ValentineUI(dear, sincerely)
    ...
    ui.DFrame(function(p)
        ...
        function p:Paint(w, h)
            ...
            surface.DrawTexturedRect(0, 0, w, h - 50)
        end
        ...
        ui.SLabel(function(p)
            p:Dock(BOTTOM)
            p:SetContentAlignment(5)
            p:SetTall(24)
            p:SetFont(Font.Arial21_1000)
            p:SetText("You won't be able to change your valentine for at least a week!")
        end)

        p.Note = ui.SLabel(function(p)
            p:Dock(BOTTOM)
            p:SetContentAlignment(5)
            p:SetFont(Font.Arial19_1000)
            p:SetTall(22)
        end)
        ...
        ui.Panel(function(p)
            ...
            ValentineFrame.Button1 = ui.DButton(function(p)
                p:Dock(LEFT)
                p:SetWide(80)
            end)

            ValentineFrame.Button2 = ui.DButton(function(p)
                p:Dock(RIGHT)
                p:SetWide(80)
            end)
        end)
