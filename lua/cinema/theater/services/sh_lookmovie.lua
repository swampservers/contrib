...
function SERVICE:GetKey(url)
    ...
    if string.match(url.encoded, "lookmovie%d*%.%w+/%w+/play/(.+)") then return url.scheme .. "://" .. url.host .. url.path .. string.match(url.encoded,"(#.+%-%d+)") end --it's apparently fine for sites to just violate URI grammar set by RFC 2396 (putting fragments before queries)
