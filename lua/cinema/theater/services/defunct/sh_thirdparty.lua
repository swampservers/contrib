...
function SERVICE:GetKey(tbl)
    ...
    if t and t["referer"] and t["key"] then
        if urlparse.parse2(t["referer"]).scheme and urlparse.parse2(t["key"]).scheme then return tbl.encoded end
    end
...
if CLIENT then
    local function LoadCustomPage(panel, js, k)
        ...
        if string.find(k, ".m3u8") then
            url = "https://swamp.sv/s/cinema/hls.html"
        end
        ...
        http.Fetch(url, function(body)
            ...
            print("Failed to fetch player page: " .. err)
        end)
    end
    ...
    function SERVICE:GetHost(Video)
        ...
        return urlparse.parse2(t["key"]).host
    end
