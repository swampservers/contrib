...
if CLIENT then
    ...
    function SERVICE:LoadVideo(Video, panel)
        if self:IsMature(Video) then
            local curURL = panel:GetURL()

            if curURL and string.StartWith(curURL, "https://swamp.sv/s/cinema/youtube.html") then
            ...
            timer.Create("youtubemature" .. tostring(math.random(1, 100000)), .1, 30, function()
                if IsValid(panel) then
                    if enabled then
                        panel:RunJavascript(payload)
                        panel:RunJavascript("if(window.ytcfg)gmod.loaded();")
                    end
            ...
            if curURL and string.StartWith(curURL, "https://www.youtube.com/embed/") then
                theater.LoadVideo(Video, true)
            end
