...
function PPM.PrePonyDraw(ent, localvals)
    ...
    if IsValid(LocalPlayer()) and LocalPlayer():Nick()=="Joker Gaming" and NEWPONYZ==nil then NEWPONYZ=true end
...
function SS_PPM_SetSubMaterials(ent)
    hook.Remove("PrePlayerDraw", "pony_draw")
    RP_PUSH("ponydraw")
...
function PPM_CLONE_MATERIALS(tab)
    local function PPM_CLONE_MATERIAL(mat)
        ...
        for k,v in pairs(kvs) do
            ...
            if kvs[k]~=nil then assert(type(kvs[k])=="string") end
        end
