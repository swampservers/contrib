...
ENT.PlacementSettings.TextureBlacklist = {"**empty**","TOOLS/TOOLSBLACK","**studio**"} --**studio** is any model
...
if(!self.PlacementSettings.AllowFloor)then
...
function ENT:FindSuitableCastOrigin()
	...
	if(MAGICBUTTON_CACHED_ORIGINPOINTS == nil)then
		...
		for _,area in pairs(navareas)do
			...
			if(area:IsUnderwater() or area:HasAttributes(NAV_MESH_AVOID) or toosmall)then
			continue
			end
			...
			if(!self:IsTraceValid(trace))then
			continue
			end
			...
			if((!self.PlacementSettings.RequireCeiling or (trace.Hit and self:IsTraceValid(trace))) and !trace.StartSolid or (trace.StartSolid and trace.FractionLeftSolid < 1))then
...
function ENT:FindCastBox(casttrace)
	...
	if(!self.PlacementSettings.AllowFloor and self.PlacementSettings.RequireCeiling)then randir.z = math.abs(randir.z) end
	...
	local traceorigin = LerpVector(math.Rand(0.1,0.4),casttrace.StartPos ,casttrace.HitPos)
		...
		if(self:IsTraceValid(trace2))then
			...
			for k,v in pairs(navareas)do
				if(v:IsUnderwater() or !v:Contains( trace2.HitPos))then
				navareas[k] = nil
				end
			end
...
function MoneyPrize() -- this is where we generate a random amount of points to give the player
...
local MagicButtonOutcomes = {
	{func = GiveMysteryBoxItem,message = "and %s",weight=1},
	{func = MagicOutcomeBountyAndPrize,message = "and won %s points and also a %s point bounty on themself!",weight=1},
	{func = MagicOutcomeKleinerFanclub,message = "and won the attention of every kleiner on the map!",weight=1},
	{func = MagicOutcomeKleinerHatred,message = "and said an anti-kleiner slur! watch out!",weight=1},
	{func = MagicOutcomeRandomTeleport,message = "and teleported somewhere mysterious",weight=1},
	{func = MagicOutcomeKick,message = "and was kicked from the server",weight=1},
}
...
if(BotSayGlobal)then BotSayGlobal("[fbc]"..activator:Nick().. " pressed a hidden button "..message)
else
