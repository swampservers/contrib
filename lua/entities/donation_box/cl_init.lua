...
function ENT:DrawTranslucent()
	...
	cam.Culled3D2D( self:GetPos() + Vector(0,0,100) + self:GetAngles():Forward()*(-6 + 1), self:LocalToWorldAngles(Angle(0,90,90)), 0.1,
        function()
        local ox,oy = 0,0
        if self:GetLibtarded() then
            ox = 500
...
net.Receive("DonationBoxPoints", function()
    ...
    vgui("DFrame", function(p)
        ...
        vgui("DLabel", function(p)
            ...
            p:SetText(libtard and
[[ joe biden is raising points to promote racial equity and LGBTQ rights
...
contribute now! the top 10 donors will have the exclusive honor of
                    being a joe biden ally!]] or
[[     Donald J. Trump is raising points to fight the Radical Far Left Democrats,
...
[[NO REFUNDS!!!!]])
        ...
        vgui("DLabel", function(p)
            ...
            -- p:SetColor(color_black)
            p:SetText(
[[(disclaimer: this is not a donation to ]].. (libtard and "any real-world political\n" or "the real-world Donald Trump\n")..
