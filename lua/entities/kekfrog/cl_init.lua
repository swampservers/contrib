...
net.Receive("KekOffer",function(len)
    ...
    Derma_StringRequest(
        "Kek offering", 
        "Enter the number of points to sacrifice.\nMay increase the income of this idol.",
        "1000",
        function(text) 
...
hook.Add("HUDPaint","saykek2",function()
    ...
    if td < 10 then 
        ...
        for i,v in ipairs(saykekdata) do
            local p = v[2]
            if IsValid(v[1]) and not v[1]:IsDormant() then p=v[1]:GetPos() end
            local sc = p:ToScreen()
            if sc.visible then
...
hook.Add("PostDrawTranslucentRenderables", "EntityUseMenu", function(d,sky,sky3d)
    ...
    if IsValid(USEMENUENT) then
        ...
        cam.Start3D2D( c, a, scl )
            ...
            draw.SimpleText( inc>0 and ("Generating "..inc.." points/minute") or "Bring to an unsafe area aboveground to generate income", "Trebuchet18", 0, -40, color_white, TEXT_ALIGN_CENTER )
        ...
        if RELEASING_USEMENU then USEMENUENT = nil RELEASING_USEMENU=false end
    end
end)
