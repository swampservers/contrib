...
vgui.Register("SLabel", {
    Init = function(self)
        self.text = "" self.font="Default" self.color=color_white self.xalign=TEXT_ALIGN_CENTER self.yalign=TEXT_ALIGN_CENTER self.newlinespacing=0
    end,
    PerformLayout = function(self, w, h)
        ...
        if self.stretch then self:SetTall(th) h=th end

        local x,y = 0,0
        if self.xalign==TEXT_ALIGN_CENTER then x=math.floor(w/2) elseif self.xalign==TEXT_ALIGN_RIGHT then x=w end
        if self.yalign==TEXT_ALIGN_CENTER then y=math.floor(h/2) elseif self.yalign==TEXT_ALIGN_BOTTOM then y=h end
        self.Paint = function() draw(x,y,self.color) end
    end,
    SetText = function(self, text)
        if self.text~=text then self.text = text
    ...
    SetContentAlignment = function(self, align)
        self.xalign = ({[0]=TEXT_ALIGN_LEFT,TEXT_ALIGN_CENTER,TEXT_ALIGN_RIGHT})[ (align-1)%3 ]
        self.yalign = ({[0]=TEXT_ALIGN_BOTTOM,TEXT_ALIGN_CENTER,TEXT_ALIGN_TOP})[math.floor((align-1)/3)]
        self:InvalidateLayout()
    end,
