...
function GetTextSize(font, text)
    ...
    if not c[text] then
        ...
        c[text] = {surface.GetTextSize(text)}
...
local function parse_settings(setting_str)
    ...
    if settings.weight then i=3 end
...
Font = defaultdict(function(setting_str) surface.CreateFont(setting_str, parse_settings(setting_str)) return setting_str end)
