...
list.Set("DesktopWindows", "PlayerEditor", {
    ...
    init = function(icon, window)
        ...
        sheet:Dock(RIGHT)
        sheet:SetSize(430, 0)
        local nof = vgui.Create("DLabel", window)
        nof:SetText("")
        nof:SetPos(50, 30)
        nof:SetSize(420, 30)
        local DermaButton = vgui.Create("DButton", window)
        DermaButton:SetText("Apply and Respawn")
        DermaButton:SetPos(150, 660)
        ...
        local function UpdateFromConvars()
            ...
            if not found then
                ...
                nof:SetText("You can't use a default model because you have a custom model applied in Pointshop.")
            end
...
concommand.Add("customize", function()
    ...
    -- Make the window
    SwampPlayerModelWindow = vgui.Create("DFrame")
    SwampPlayerModelWindow:SetSize(newv.width, newv.height)
    SwampPlayerModelWindow:SetTitle(newv.title)
    SwampPlayerModelWindow:Center()
    SwampPlayerModelWindow:SetMouseInputEnabled(true)
    SwampPlayerModelWindow:SetKeyboardInputEnabled(true)
    SwampPlayerModelWindow:MakePopup()
    newv.init(nil, SwampPlayerModelWindow)
end)
