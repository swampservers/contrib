...
API_PlayerHandles = API_PlayerHandles or {}

function PlayerHandle(userid)
    local h = API_PlayerHandles[userid]

    if not h then
        -- not slow because of playerbysteamid.lua
        h = {
            player = Player(userid),
            callbacks = {},
            OnLoad = function(handle, func)
                if IsValid(handle.player) then
                    func(handle.player)
                else
        ...
        API_PlayerHandles[userid] = h
    end

    return h
end

hook.Add("OnEntityCreated", "PlayerHandle", function(ent)
    if ent:IsPlayer() then
        local h = PlayerHandle(ent:UserID())
        h.player = ent
...
for i, v in ipairs({"API_DATALEN", "API_ANY", "API_NIL", "API_BOOL", "API_FLOAT", "API_DOUBLE", "API_INT8", "API_INT16", "API_INT32", "API_UINT8", "API_UINT16", "API_UINT32", "API_STRING", "API_NT_STRING", "API_VECTOR", "API_COMP_VECTOR", "API_ANGLE", "API_COMP_ANGLE", "API_ENTITY", "API_PLAYER_HANDLE", "API_LIST", "API_TABLE",}) do
...
local API_Readers = {
    ...
    [API_NETWORK_STRING] = function(v)
        ...
        local st = NetworkStringCache[id]

        if not st then
            st = util.NetworkIDToString(id)
            assert(st ~= nil)
            NetworkStringCache[id] = st
        end
    ...
    [API_PLAYER_HANDLE] = function() return PlayerHandle(net.ReadUInt(16)) end,
    [API_LIST] = function()
        API_Read({API_ANY})
    end,
    [API_TABLE] = function()
...
local API_Writers = {
    ...
    [API_ANY] = function(v)
        ...
        net.WriteUInt(API_TYPESIZE, typ)
        API_Write(typ, v)
    end,
...
function API_Command(name, argtypes, client_function, unreliable)
    ...
    if SERVER then
        ...
        player[name] = function(ply, ...)
            ...
            for i = 1, nargs do
                print(argtypes[i], argvals[i], API_Write)
...
API_Command("DoTheThing", {
    API_UINT8, {API_STRING},
    API_PLAYER_HANDLE
}, function(v, strings, ply)
    print(v)
    PrintTable(strings)

    ply:OnLoad(function(ply)
        print("PLAYERLOADED", ply)
    end)
end)
