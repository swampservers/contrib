...
local API_Readers = {
    ...
    [API_PLAYER_HANDLE] = function()
        return PlayerHandle(net.ReadUInt(16))
    end,
...
local API_Writers = {
    ...
    [API_PLAYER_HANDLE] = function(v)
        if istable(v) then v = v.player end
        local id = v:UserID()
        assert(id <= 65535)
        net.WriteUInt(id, 16)
    end,
...
API_Command("DoTheThing", {API_UINT8, {API_STRING}, API_PLAYER_HANDLE}, function(v, strings, ply)
    print(v)
    PrintTable(strings)
