...
function GMABlacklist(fpath, wsid, fhandle)
    ...
    for i = 1, 8192 * 2 do
        ...
        if not entry then
            if err then
                dbge("GMABlacklist", "enumfiles", wsid, err)
            end
    ...
    if not next(check_vtfs) then
        dbgn(3, "CheckVTF", "none found??")
    end
    ...
    for i = 1, #paths do
        local path = paths[i]
        --Check 1: modules
        if path:find("includes", 4, true) and path:gsub("\\", "/"):gsub("/./", "/"):gsub("/./", "/"):gsub("/+", "/"):find("lua/includes/", 1, true) then return nil, "includes" end
        --Check 2
