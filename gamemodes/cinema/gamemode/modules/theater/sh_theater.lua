...
if SERVER then
    util.AddNetworkString("ThumbnailDelivery")
end
...
function THEATER:VideoTime()
    if not self:IsPlaying() or self:VideoDuration() == 0 then return "" end
...
function THEATER:Think()
    ...
    if SERVER then
        if not self:IsPlaying() and (not self:IsPlayingDefault() or not self:IsQueueEmpty()) then
            self:NextVideo()
        end
        --[[
	else
...
if SERVER then
    ...
    function THEATER:NextVideo()
        ...
        if self:IsQueueEmpty() then
            ...
            if Video:GetOwnerName() ~= "" then
                self:AnnounceToPlayers({'Theater_VideoRequestedBy', Video:GetOwnerName()})
            end
    ...
    function THEATER:RequestVideo(ply, url, force)
        ...
        if (ply.LastVideoRequest or 0) + 0.3 > CurTime() then return end
        ...
        if self:IsPrivate() then
            ...
            if self:IsQueueLocked() and ply ~= self:GetOwner() then return self:AnnounceToPlayer(ply, 'Theater_OwnerLockedQueue') end
        end
        ...
        for _, vid in pairs(self:GetQueue()) do
            if vid:Type() == info.type and vid:Key() == info.key then
        ...
        self:AnnounceToPlayer(ply, {'Theater_ProcessingRequest', vid:Service():GetName()})
    ...
    function THEATER:Seek(seconds)
        ...
        if not tonumber(seconds) then
            ...
            seconds = tonumber(hr) * 3600 + tonumber(min) * 60 + tonumber(sec)
        end
    ...
    function THEATER:NumRequiredVoteSkips()
        ...
        if self._Name == "Movie Theater" then
            ratio = 0.7
        end
