...
if CLIENT then
	...
	function SERVICE:LoadVideo( Video, panel )
		...
		timer.Create("nekoupdate"..tostring(math.random(1,100000)),1,45,function()
			if IsValid(panel) then
				...
				if panel.phase == 0 then
					panel:RunJavascript("var nekoparent=document.getElementById('neko');console.log(nekoparent.getElementsByTagName('input').length);console.log('LOGIN:'+nekoparent.getElementsByClassName('message').length);") --check if already logged in
		...
		function panel:ConsoleMessage(msg)
			if msg then
				if (LocalPlayer().videoDebug) print(msg) end --required for debug in this case
