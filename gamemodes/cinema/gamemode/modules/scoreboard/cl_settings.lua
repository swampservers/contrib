...
function SETTINGS:Create()
    ...
    addLabel('Display')
    local Wrap = vgui.Create("Panel", self)
    Wrap:DockPadding(16, 0, 16, 0)
    Wrap:SetTooltip("Video playback quality; affects FPS.\nSettings are 256p/512p/1024p")
    ...
    DComboBox:AddChoice("Low (Best FPS)")
    DComboBox:AddChoice("Medium")
    DComboBox:AddChoice("High")
    -- DComboBox:AddChoice( "Extreme (LOW FPS)" )
    DComboBox:ChooseOptionID(math.Clamp(GetConVar("cinema_quality"):GetInt() + 1, 1, 3))

    DComboBox.OnSelect = function(self, index, value)
        RunConsoleCommand("cinema_quality", tostring(index - 1))
    end
